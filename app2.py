...
def video_page(video_id):
    print("Getting video")
    session = get_cassandra_connection()
    video = session.execute("SELECT * FROM videos WHERE id = %s", (uuid.UUID(video_id),)).one()
    if video:
...
def search_frames(video_id):
    ...
    video = session.execute("SELECT * FROM videos WHERE id = %s", (uuid.UUID(video_id),)).one()
    if video:
        frames = list(session.execute("SELECT * FROM frame_embeddings WHERE video_id = %s", (uuid.UUID(video_id),)))
        ...
        results = sorted(zip(frame_timestamps, similarities[:, 0]), key=lambda x: x[1], reverse=True)[:5]
        print(results)
